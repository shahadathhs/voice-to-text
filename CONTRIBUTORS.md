# Contributors

Thank you to all the people who have contributed to this project!

## Project Lead

- **[Shahadath Hossain](https://github.com/shahadathhs)** - Creator and maintainer

## Core Contributors

Contributors who have made significant contributions to the project:

<!-- Add contributors here with permission -->

## Code Contributors

This project exists thanks to all the people who contribute code.

<a href="https://github.com/shahadathhs/voice-to-text/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=shahadathhs/voice-to-text" />
</a>

## How to Become a Contributor

We welcome contributions from everyone! See [CONTRIBUTING.md](CONTRIBUTING.md) for details.

### Ways to Contribute

- 🐛 **Report bugs**: Open an issue with details
- 💡 **Suggest features**: Share your ideas
- 📝 **Improve docs**: Fix typos, add examples
- 🔧 **Submit PRs**: Fix bugs, add features
- 🎨 **Improve design**: UI/UX improvements
- 📣 **Spread the word**: Share the project
- 💬 **Help others**: Answer questions in issues

### Recognition

Contributors who make significant contributions will be:
- Listed in this file
- Mentioned in release notes
- Credited in the README
- Eligible for project maintainer role

## Contributor Stats

- **Total Contributors**: See GitHub contributors graph
- **Total Commits**: See GitHub activity graph
- **Total Stars**: ⭐ on GitHub
- **Total Forks**: See GitHub repository

## Emeritus Contributors

Contributors who are no longer active but made important contributions:

<!-- Add emeritus contributors here -->

## Special Thanks

- **OpenAI** for creating Whisper
- **SpeechBrain** for the speaker diarization models
- **FastAPI** for the excellent web framework
- **UV** for the lightning-fast package manager

## Contact

For questions about contributing, see:
- [CONTRIBUTING.md](CONTRIBUTING.md)
- [SUPPORT.md](SUPPORT.md)
- [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)

---

**Want to contribute?** Check out [CONTRIBUTING.md](CONTRIBUTING.md) to get started!
